...
export function RangeSlider({
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex h-10 w-full touch-none select-none items-center overflow-hidden rounded-lg bg-neutral-100",
    ...
    >
      ...
      <motion.div
        className="absolute inset-y-0 left-0 bg-brand-500/25"
        style={{ width: left }}
      />
      ...
      <div className="pointer-events-none absolute inset-x-2 inset-y-0">
        {ticks.map((t) => {
          ...
          return (
            <span
              key={t}
              className="absolute top-1/2 size-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-neutral-dark-900/25"
              style={{ left: `${tp}%` }}
            />
          );
        })}
      ...
      <motion.div
        ...
        className="absolute top-1/2 h-5 w-1.5 rounded-sm bg-brand-500 shadow-sm outline-none ring-brand-500/30 focus-visible:ring-4"
