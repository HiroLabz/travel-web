...
export interface SelectTriggerProps {
  className?: string;
  children: ReactNode;
  /** Overrides the auto-generated trigger id — set this to pair with an external <Label htmlFor>. */
  id?: string;
}
...
export function SelectTrigger({ className, children, id }: SelectTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative z-10 flex w-full items-center justify-between gap-2 rounded-[10px] border border-neutral-100 bg-brand-subtle px-4 py-4 text-sm text-neutral-dark-900 outline-none transition-colors",
        "hover:border-neutral-300 focus-visible:ring-2 focus-visible:ring-brand-500",
...
export function SelectValue({ placeholder, className }: SelectValueProps) {
  ...
  return (
    <span
      className={cn(label ? "text-neutral-dark-900" : "text-neutral-600", className)}
    >
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  // Move focus into the listbox on open, landing on the selected option (or
  // the first one) so keyboard users don't need an extra Tab press.
  useEffect(() => {
    if (!open) return;
    const options = listboxRef.current?.querySelectorAll<HTMLButtonElement>('[role="option"]');
    if (!options || options.length === 0) return;
    const selected = listboxRef.current?.querySelector<HTMLButtonElement>('[role="option"][aria-selected="true"]');
      ...
      <button
        ...
        className={cn(
          ...
          selected
            ...
            : "text-neutral-dark-900 hover:bg-brand-subtle focus-visible:bg-brand-subtle",
          "disabled:pointer-events-none disabled:opacity-50",
          className,
        )}
      >
