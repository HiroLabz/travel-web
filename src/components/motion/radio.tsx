  ...
  return (
    ...
    >
      <motion.button
        ...
        className={cn(
          "relative inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-[1.5px] outline-none transition-colors duration-200",
      ...
      >
        {selected ? (
          <motion.span
            layoutId={layoutId}
            className="absolute inset-1 rounded-full bg-brand-500"
            transition={reduce ? { duration: 0 } : SPRING_LAYOUT}
          />
        ) : null}
      </motion.button>
      {label ? (
        <span className={cn("select-none text-sm text-neutral-dark-900", disabled && "opacity-60")}>
          {label}
        </span>
      ) : null}
    </label>
